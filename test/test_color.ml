(** Tests for the {!Color} module.
    ...
    {1 Module Overview}
    [Color] provides a simple RGB triple with components in [0.0, 1.0] and
    a small algebra for constructing and scaling colours.

    {1 Invariants}
    {2 [make]}
    - [make r g b] stores [r], [g], [b] in the [r], [g], [b] fields
    ...
    {1 Notes}
    Components outside [0,1] are not clamped by the implementation; the
    caller is responsible for supplying sensible values. *)
...
let test_shade_black_invariant _ =
  List.iter (fun s ->
    ...
    assert_color_eq ~msg:(Printf.sprintf "shade %.1f black" s)
      (0.0, 0.0, 0.0) result
...
(** Associativity-like property: [(s *. (t *. c)) = (s *. t) *. c]
    (since [*. = shade] and shade composes multiplicatively). *)
