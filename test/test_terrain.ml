(** Tests for the {!Terrain} module.
    ...
    {1 Module Overview}
    [Terrain] converts 2-D Perlin noise into a layered voxel landscape. It
    ...
    - Bounded: since [perlin2d ∈ [-1,1]], the formula [⌊6 + 4·n⌋] with
      [n ∈ [-1,1]] gives heights in [{2,…,10}]. (Using [int_of_float] which
      truncates toward zero, so the range may be [1..10] in edge cases; tests
      allow for this.)

    {2 [fill_chunk]}
    - Returns exactly [chunk_size³] blocks.
    - Layering law (for a block at world coordinate [(wx, wy, wz)] with surface
      height [h = height_at wx wz]):
    - [wy > h] → [Air]
    - [wy = h] → [Grass]
    - [h-3 ≤ wy < h] → [Dirt]
    - [wy < h-3] → [Stone]
    - A chunk entirely above the tallest possible surface (cy = 5, world-y ≥ 80)
      contains only [Air].
    - A chunk deep underground (cy = -2, world-y ≤ -17) contains only [Stone].
*)
...
let test_height_range _ =
  for x = -10 to 10 do
    for z = -10 to 10 do
      ...
      assert_bool (Printf.sprintf "height_at(%d,%d)=%d >= 1" x z h) (h >= 1);
...
(** [height_at] returns an integer (type-level guarantee, exercised here by
    checking it is a valid array index within [0, cs-1]). *)
...
(** Chunk at cy = -2 spans world-y = -32..-17. Surface max = 10, so every block
    in this chunk is at least 27 blocks below the surface. *)
...
(** The chunk at cy = 0 (world-y 0..15) includes the surface for most columns
    and therefore contains Grass, Dirt, and Stone. *)
let test_fill_chunk_has_grass _ =
  let blocks = Terrain.fill_chunk ~cx:0 ~cy:0 ~cz:0 in
  assert_bool "cy=0 has Grass" (Array.exists (fun b -> b = Block.Grass) blocks)

let test_fill_chunk_has_dirt _ =
  let blocks = Terrain.fill_chunk ~cx:0 ~cy:0 ~cz:0 in
  assert_bool "cy=0 has Dirt" (Array.exists (fun b -> b = Block.Dirt) blocks)

let test_fill_chunk_has_stone _ =
  let blocks = Terrain.fill_chunk ~cx:0 ~cy:0 ~cz:0 in
  assert_bool "cy=0 has Stone" (Array.exists (fun b -> b = Block.Stone) blocks)
...
(** [fill_chunk] agrees with [height_at]: the block at world-y = h is [Grass]
    for every column in the chunk at cy = 0. *)
