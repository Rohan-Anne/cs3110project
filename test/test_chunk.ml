(** Tests for the {!Chunk} module.
    ...
    {1 Module Overview}
    A [Chunk] is a [chunk_size × chunk_size × chunk_size] cube of blocks
    stored in a flat array.  Chunks are the fundamental unit of world
    storage; the world is a hash-map from chunk coordinates to [Chunk.t].

    {1 Invariants}
    {2 Indexing}
    - [Chunk.index] is a bijection from
      [{0,…,cs-1}³ → {0,…,cs³-1}] where [cs = Config.chunk_size].
    - [index 0 0 0 = 0].
    - [index (cs-1) (cs-1) (cs-1) = cs³ - 1].
    - The strides satisfy: Δx = 1, Δy = cs, Δz = cs².
      Concretely: [index x y z = x + y*cs + z*cs²].
    ...
    - A freshly-created chunk has all blocks set to whatever value was
      passed in the [blocks] array (typically [Air]).

    {2 Coordinates}
    - [Chunk.x], [Chunk.y], [Chunk.z] return the chunk's world-grid
      coordinates as stored by [create]. *)
...
let test_index_z_stride _ =
  for x = 0 to cs - 1 do
    for y = 0 to cs - 1 do
      ...
      assert_equal ~msg:(Printf.sprintf "z-stride at x=%d y=%d" x y)
        (cs * cs) diff
    done
  done
...
let test_index_bijection _ =
  ...
  assert_equal ~msg:"bijection: all indices unique" (cs * cs * cs) (Array.length unique)
...
let test_no_aliasing_corners _ =
  ...
  let far = cs - 1 in
  assert_equal ~msg:"far corner unaffected" Block.Air
...
(** Positive, zero, and negative chunk coordinates are all stored
    faithfully by [create] and returned unchanged by [x/y/z]. *)
let test_coords_positive _ =
  let c = Chunk.create ~x:3 ~y:5 ~z:7
            ~blocks:(Array.make (cs*cs*cs) Block.Air) in
...
let test_coords_independent _ =
  let c = Chunk.create ~x:10 ~y:20 ~z:30
            ~blocks:(Array.make (cs*cs*cs) Block.Air) in
