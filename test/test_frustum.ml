(** Tests for the {!Frustum} module.
    ...
    {ul
     ...
     {- For a standard OpenGL perspective × identity-view MVP (camera at the
        ...
        - An AABB straddling [(0, 0, −10)] (well inside view volume) is reported
          inside.
        - An AABB at [(0, 0, +10)] (behind the camera, past the near plane) is
          reported outside.
...
let std_mvp () =
  Math3d.perspective ~fov_y_radians:(pi /. 3.0) ~aspect:1.0 ~near:0.1 ~far:100.0
...
let test_of_mvp_does_not_raise _ =
  (* Even a singular matrix (all zeros) shouldn't raise; degenerate planes are
     allowed by the RI. *)
...
(** Containment: a point known to be inside the frustum forces any AABB covering
    it to be reported inside. *)
...
(** Sanity check: degenerate min=max AABB at an interior point is treated as
    inside (the half-spaces all evaluate >= 0 at that point). *)
...
(** Property: any AABB that contains the known-interior point [(0, 0, −10)] must
    be reported inside, regardless of how big or oddly placed it is. This
    catches sign errors in plane extraction and AABB orientation. *)
let qcheck_aabb_containing_known_point =
  QCheck2.Test.make ~name:"aabb_containing_in_view_point_is_inside" ~count:1000
    (QCheck2.Gen.quad arb_coord arb_coord arb_coord arb_half)
    (fun (ox, oy, oz, half) ->
      let f = Frustum.of_mvp (std_mvp ()) in
      let inside_pt_x = 0.0 and inside_pt_y = 0.0 and inside_pt_z = -10.0 in
      (* extend half-extents far enough that the AABB is guaranteed to cover the
         interior point regardless of the random offset *)
