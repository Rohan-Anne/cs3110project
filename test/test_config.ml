(** Tests for the {!Config} module.
    ...
    parameters, world geometry, player dimensions, and physics values.
    Every value is a module-level constant — there are no functions.

    {1 Invariants}
    - [move_speed] and [sprint_speed] are both positive, with
      [move_speed < sprint_speed].
    - [mouse_sensitivity] is positive.
    - [pitch_limit] is positive and strictly less than [π/2]
      (prevents the camera from flipping at the poles).
...
(** Convenience: assert two floats are exactly equal (no epsilon needed
    for literal constants that are stored as-is). *)
...
let test_fov_y_value _ =
  assert_feq ~msg:"fov_y = pi/3" (pi /. 3.0) Config.fov_y

let test_near_value _ =
  assert_feq ~msg:"near = 0.1" 0.1 Config.near

let test_far_value _ =
  assert_feq ~msg:"far = 1000.0" 1000.0 Config.far
...
let test_gravity_value _ =
  assert_feq ~msg:"gravity = 20.0" 20.0 Config.gravity
...
let test_near_positive _ =
  assert_bool "near > 0" (Config.near > 0.0)

let test_far_positive _ =
  assert_bool "far > 0" (Config.far > 0.0)
...
let test_gravity_positive _ =
  assert_bool "gravity > 0" (Config.gravity > 0.0)
...
(** Jump velocity must be enough to overcome at least one gravity step,
    i.e. jump_velocity > 0. (Covered by sign invariant, but restated here
    for clarity.) *)
let test_jump_overcomes_gravity_step _ =
  let dt = 1.0 /. 60.0 in (* one frame at 60 fps *)
