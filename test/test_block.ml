(** Tests for the {!Block} module.
    ...
    {1 Invariants}
    - The four variants [Air], [Stone], [Dirt], and [Grass] are distinct: no two
      different constructors compare as equal.
    ...
    [Buffer], [Gl_utils], [Shader], and [Window] all require a live OpenGL / SDL
    context and cannot be exercised in a headless test runner. Their correctness
    is verified through integration runs of the actual game. *)
...
let test_air_ne_stone _ = assert_bool "Air <> Stone" (Block.Air <> Block.Stone)
let test_air_ne_dirt _ = assert_bool "Air <> Dirt" (Block.Air <> Block.Dirt)
let test_air_ne_grass _ = assert_bool "Air <> Grass" (Block.Air <> Block.Grass)
...
let test_stone_solid _ = assert_bool "Stone is solid" (is_solid Block.Stone)
let test_dirt_solid _ = assert_bool "Dirt is solid" (is_solid Block.Dirt)
let test_grass_solid _ = assert_bool "Grass is solid" (is_solid Block.Grass)
...
let test_ne_symmetric _ =
  ...
  List.iter
    (fun (a, b) ->
      assert_bool
        (Printf.sprintf "%s <> %s implies %s <> %s" (name a) (name b) (name b)
           (name a))
        (b <> a))
    pairs
