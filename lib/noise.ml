...
(* To avoid index wrapping, we double every value in the array *)
let p = Array.init 512 (fun i -> perm.(i mod 256))
...
let perlin2d x z =
  ...
  let v = fade zf in
  let aa = p.(p.(xi) + zi) in
  let ab = p.(p.(xi) + zi + 1) in
  let ba = p.(p.(xi + 1) + zi) in
  let bb = p.(p.(xi + 1) + zi + 1) in
  (* Interpolate *)
  lerp
