...
let perlin2d x z =
  ...
  let u = fade xf in
  let v = fade zf in
  let xi1 = (xi + 1) land 255 in
  let zi1 = (zi + 1) land 255 in
  let aa = perm.((perm.(xi) + zi) land 255) in
  let ab = perm.((perm.(xi) + zi1) land 255) in
  let ba = perm.((perm.(xi1) + zi) land 255) in
  let bb = perm.((perm.(xi1) + zi1) land 255) in
  (* Interpolate *)
  lerp
