...
(* AF: [{window; gl_context}] represents an open SDL2 window with an active
   OpenGL 3.3 Core context. [window] is the SDL window handle and [gl_context]
   is the OpenGL context bound to that window. RI: [window] and [gl_context] are
   live (not yet destroyed) SDL/GL handles, and [gl_context] is current on the
   calling thread. *)
