...
(* AF: [{x; y; z; blocks}] represents the cubic region of the voxel world
        whose chunk-grid corner is [(x, y, z)]. The block at local position
        [(bx, by, bz)] with [0 <= bx, by, bz < Config.chunk_size] is
        [blocks.(bx + by * chunk_size + bz * chunk_size * chunk_size)].
